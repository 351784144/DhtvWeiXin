...
class FormDateTime extends ZfFormDateTime
{
    ...
    public function __invoke(ElementInterface $element = null)
    {
        ...
        if ($element) {
            ...
            // bootstrap3 config;
            $bootstrapConfig = isset($config[$this->configKey]) ? $config[$this->configKey] : array();

            // 检查是否需要用到bootstrapDatetimepicker
            if ($this->checkDatetimepicker($bootstrapConfig)) {

                // js通过id激活Datetimepicker，如果表单为设置id则将name设置为id
                $this->setId($element);
                // 添加bootstrapDatetimepicker的js,css
                $this->addDatetimepicker($element, $bootstrapConfig);
            }

            // 设置 验证和格式化的 时间格式
            $this->setDateFormat($bootstrapConfig);
            // 格式化format
